...
export default function Home({ params }: { params: Promise<{ locale: string }> }) {
  ...
  async function handleSubmit(e?: React.FormEvent, overrideQuery?: string, deepDive = false) {
    ...
    // Soft auth gate — check if anonymous user has used their free queries
    if (!user) {
      const used = loadFromStorage<number>("pc_free_queries", 0);
    ...
    try {
      ...
      // Increment free query counter for anonymous users
      if (!user) {
        const used = loadFromStorage<number>("pc_free_queries", 0);
        saveToStorage("pc_free_queries", used + 1);
      }
    }
  }
            ...
            {user ? (
              <div className="rounded-2xl p-4 mb-4" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                ...
                <div className="mt-3 pt-3" style={{ borderTop: "1px solid var(--border)" }}>
                  <p className="text-xs font-semibold mb-1" style={{ color: "var(--text)" }}>{t.linkedAccounts}</p>
                  <p className="text-xs mb-3" style={{ color: "var(--text-muted)" }}>{t.linkedAccountsDesc}</p>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs" style={{ color: "var(--text)" }}>Google</span>
                    </div>
                    {linkedProviders.includes("google") ? (
                      <span className="text-xs font-medium" style={{ color: "var(--emerald-700)" }}>{t.linked}</span>
                    ) : (
                      <button onClick={() => linkAccount("google")} className="text-xs font-medium px-2 py-1 rounded-lg" style={{ border: "1px solid var(--border)", color: "var(--primary)" }}>{t.linkGoogle}</button>
                    )}
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" style={{ color: "var(--text)" }}><path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.09l.01-.01zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/></svg>
                      <span className="text-xs" style={{ color: "var(--text)" }}>Apple</span>
                    </div>
                    {linkedProviders.includes("apple") ? (
                      <span className="text-xs font-medium" style={{ color: "var(--emerald-700)" }}>{t.linked}</span>
                    ) : (
                      <button onClick={() => linkAccount("apple")} className="text-xs font-medium px-2 py-1 rounded-lg" style={{ border: "1px solid var(--border)", color: "var(--primary)" }}>{t.linkApple}</button>
                    )}
                  </div>
                </div>
              </div>
          ...
          <div className="rounded-3xl p-8 max-w-sm w-full slide-up elevation-3" style={{ background: "var(--surface)" }}>
            <div className="text-center mb-6">
              ...
              <p className="text-sm" style={{ color: "var(--text-muted)" }}>
                {lang === "ko"
                  ? "모든 기능을 계속 사용하려면 무료 계정을 만드세요. 상담 기록, 발달 추적, 실행 계획이 저장됩니다."
                  : "Create a free account to keep your advice, track milestones, and save your progress across devices."}
              </p>
            </div>

            {/* Google */}
            <button onClick={() => signInWithGoogle()} className="w-full py-3 rounded-xl text-sm font-medium mb-2 flex items-center justify-center gap-2" style={{ border: "1px solid var(--border)", background: "#fff", color: "#3c4043" }}>
              <svg width="18" height="18" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
              {lang === "ko" ? "Google로 계속하기" : "Continue with Google"}
            </button>

            {/* Apple */}
            <button onClick={() => signInWithApple()} className="w-full py-3 rounded-xl text-sm font-medium mb-3 flex items-center justify-center gap-2" style={{ border: "1px solid var(--border)", background: "#000", color: "#fff" }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.09l.01-.01zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/></svg>
              {lang === "ko" ? "Apple로 계속하기" : "Continue with Apple"}
            </button>
            ...
            <div className="space-y-2">
              ...
              >
                {authLoading ? "..." : (lang === "ko" ? "무료 가입하기 →" : "Sign Up Free →")}
              </button>
            </div>
            ...
            <p className="text-xs text-center mt-4" style={{ color: "var(--text-muted)" }}>
              ...
              <button
                onClick={() => { setShowAuthGate(false); setAuthMode("login"); setTab("profile"); }}
